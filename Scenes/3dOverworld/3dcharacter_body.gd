...
func _ready() -> void:
	move_dice = rng.randi_range(45, 110)
...
func _physics_process(delta):
	...
	if navigation_agent.is_target_reachable():
		...
		if stuck > 8:
			position = NavigationServer3D.map_get_closest_point(get_world_3d().get_navigation_map(), position)
			print("unstuck!")
		stuck += 0.1
		return
	stuck = 0
	...
	if moved > move_dice and not disabled_spawn:
		if len(get_tree().get_nodes_in_group("spawned_npc")) > max_spawns:
			moved = 0
		else:
